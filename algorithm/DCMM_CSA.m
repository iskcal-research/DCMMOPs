function [peaks, all_ps] = DCMM_CSA(D, G, L, P, R, cmp, run)
    ...
    copy = 2;                                   % the No. of the clone
    
	maxG = prob.freq / (NP * copy);
    curG = 0;
    ...
    while evals < prob.maxFEs
        ...
        if prob.rest > 0
            for i = 1:length(species)
                ...
                sub_dis = matdis(species(i).idx, species(i).idx);
                
                [next_pop, next_fit, next_con, evals, prob] = CSA1(sub_pop, sub_fit, sub_con, sub_dis, copy, evals, prob, cht, algRand);
